...
object BroadcastTest {
  def main(args: Array[String]): Unit = {
    ...
    for (i <- 0 until 3) {
      ...
      val broadArr = spark.sparkContext.broadcast(arr) //广播数组arr
      val observedSizes = spark.sparkContext.parallelize(1 to 10, slices).map(_ => broadArr.value.length) //将RDD中的每个值替换为数组的长度
